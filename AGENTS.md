...
- format: `make fmt`
- lint: `make lint`
- `//nolint` を使わない。lint の指摘は、コードまたは設定を適切に修正して解消する
...
- After modifying code, run `make fmt` after `graphify update .`; the required order is `graphify update .` → `make fmt` → `make lint`.
